...
void IRAM_ATTR disp_spi_transfer_cmd(int8_t cmd) {
    ...
    gpio_set_level(M5_PIN_NUM_DC, 0);
...
void IRAM_ATTR disp_spi_transfer_cmd_data(int8_t cmd, uint8_t *data, uint32_t len) {
	...
	gpio_set_level(M5_PIN_NUM_DC, 1);
...
static void IRAM_ATTR _TFT_pushColorRep(color_t *color, uint32_t len, uint8_t rep, uint8_t wait)
{
	...
	gpio_set_level(M5_PIN_NUM_DC, 1);								// Set DC to 1 (data mode);
...
void TFT_PinsInit()
{
    ...
    gpio_pad_select_gpio(M5_PIN_NUM_CS);
    gpio_pad_select_gpio(M5_PIN_NUM_MISO);
    gpio_pad_select_gpio(M5_PIN_NUM_MOSI);
    gpio_pad_select_gpio(M5_PIN_NUM_CLK);
    gpio_pad_select_gpio(M5_PIN_NUM_DC);

    gpio_set_direction(M5_PIN_NUM_MISO, GPIO_MODE_INPUT);
    gpio_set_pull_mode(M5_PIN_NUM_MISO, GPIO_PULLUP_ONLY);
    gpio_set_direction(M5_PIN_NUM_CS, GPIO_MODE_OUTPUT);
    gpio_set_direction(M5_PIN_NUM_MOSI, GPIO_MODE_OUTPUT);
    gpio_set_direction(M5_PIN_NUM_CLK, GPIO_MODE_OUTPUT);
    gpio_set_direction(M5_PIN_NUM_DC, GPIO_MODE_OUTPUT);
    gpio_set_level(M5_PIN_NUM_DC, 0);
#if USE_TOUCH
    ...
    gpio_set_level(M5_PIN_NUM_RST, 0);
}
...
void TFT_display_init()
{
    ...
    gpio_set_level(M5_PIN_NUM_RST, 1);
